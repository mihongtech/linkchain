		...
		for i := 0; i < len(result.entries) && pendingQueries < alpha; i++ {
			...
			if !asked[n.ID] {
				...
				go func() {
					...
					if err != nil {
						log.Trace("findnode failed", "err", err)
...
func (tab *Table) closest(target math.Hash, nresults int) *nodesByDistance {
...
func (tab *Table) bucket(sha math.Hash) *bucket {
