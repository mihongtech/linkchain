...
func TestBucket_bumpNoDuplicates(t *testing.T) {
	...
	cfg := &quick.Config{
		...
		Values: func(args []reflect.Value, rand *rand.Rand) {
			...
			nodes := make([]*Node, n)
			for i := range nodes {
				nodes[i] = nodeAtDistance(math.Hash{}, 200)
			}
...
func nodeAtDistance(base math.Hash, ld int) (n *Node) {
	n = new(Node)
...
func (*closeTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &closeTest{
		...
		Target: gen(math.Hash{}, rand).(math.Hash),
...
func TestTable_Lookup(t *testing.T) {
	self := nodeAtDistance(math.Hash{}, 0)
...
var lookupTestnet = &preminedTestnet{
	...
	targetSha: math.Hash{0x5c, 0x94, 0x4e, 0xe5, 0x1c, 0x5a, 0xe9, 0xf7, 0x2a, 0x95, 0xec, 0xcb, 0x8a, 0xed, 0x3, 0x74, 0xee, 0xcb, 0x51, 0x19, 0xd7, 0x20, 0xcb, 0xea, 0x68, 0x13, 0xe8, 0xe0, 0xd6, 0xad, 0x92, 0x61},
	dists: [257][]NodeID{
		240: {
...
type preminedTestnet struct {
	target    NodeID
	targetSha math.Hash // sha3(target)
...
func (n *preminedTestnet) mine(target NodeID) {
	n.target = target
	n.targetSha = math.HashH(n.target[:])
	found := 0
...
func sortedByDistanceTo(distbase math.Hash, slice []*Node) bool {
	var last math.Hash
	for i, e := range slice {
