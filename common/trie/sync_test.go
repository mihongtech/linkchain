...
func checkTrieContents(t *testing.T, db *Database, root []byte, content map[string][]byte) {
	...
	trie, err := New(math.BytesToHash(root), db)
	if err != nil {
		t.Fatalf("failed to create trie at %x: %v", root, err)
	}
	if err := checkTrieConsistency(db, math.BytesToHash(root)); err != nil {
...
func checkTrieConsistency(db *Database, root math.Hash) error {
...
func TestEmptyTrieSync(t *testing.T) {
	diskdbA, _ := lcdb.NewMemDatabase()
	triedbA := NewDatabase(diskdbA)

	diskdbB, _ := lcdb.NewMemDatabase()
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	queue := append([]math.Hash{}, sched.Missing(batch)...)
	for len(queue) > 0 {
...
func TestIterativeDelayedTrieSync(t *testing.T) {
	...
	queue := append([]math.Hash{}, sched.Missing(10000)...)
	for len(queue) > 0 {
...
func testIterativeRandomTrieSync(t *testing.T, batch int) {
	...
	queue := make(map[math.Hash]struct{})
...
func TestDuplicateAvoidanceTrieSync(t *testing.T) {
	...
	queue := append([]math.Hash{}, sched.Missing(0)...)
	requested := make(map[math.Hash]struct{})
...
func TestIncompleteTrieSync(t *testing.T) {
	...
	added := []math.Hash{}
	queue := append([]math.Hash{}, sched.Missing(1)...)
	for len(queue) > 0 {
