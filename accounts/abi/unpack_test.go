...
func TestUnmarshal(t *testing.T) {
	...
	helloHash := math.BytesToHash(common.RightPadBytes([]byte("hello"), 32))
	if hash != helloHash {
	...
	if outAddr[0] != (meta.AccountID{1}) {
		t.Errorf("expected %x, got %x", meta.AccountID{1}, outAddr[0])
	}
	...
	if outAddrStruct.A[0] != (meta.AccountID{1}) {
		t.Errorf("expected %x, got %x", meta.AccountID{1}, outAddrStruct.A[0])
	}
	...
	if outAddrStruct.B[0] != (meta.AccountID{2}) {
		t.Errorf("expected %x, got %x", meta.AccountID{2}, outAddrStruct.B[0])
	}
	if outAddrStruct.B[1] != (meta.AccountID{3}) {
		t.Errorf("expected %x, got %x", meta.AccountID{3}, outAddrStruct.B[1])
	}
