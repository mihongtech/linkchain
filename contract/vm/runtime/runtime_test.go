...
func TestCall(t *testing.T) {
	...
	state, _ := state.New(math.Hash{}, db)
	adapter := contract.NewStateAdapter(state, math.Hash{}, math.Hash{}, meta.AccountID{}, 0)
	address, _ := meta.HexToAccountID("0x0a")
	adapter.SetCode(address, []byte{
		byte(vm.PUSH1), 10,
		byte(vm.PUSH1), 0,
		byte(vm.MSTORE),
		byte(vm.PUSH1), 32,
	...
	ret, _, err := Call(address, nil, &Config{State: adapter})
	if err != nil {
		t.Fatal("didn't expect error", err)
	}
...
func benchmarkEVM_Create(bench *testing.B, code string) {
	var (
		...
		adapter    = contract.NewStateAdapter(statedb, math.Hash{}, math.Hash{}, meta.AccountID{}, 0)
		sender     = meta.BytesToAccountID([]byte("sender"))
		receiver   = meta.BytesToAccountID([]byte("receiver"))
	)

	adapter.CreateContractAccount(sender)
	adapter.SetCode(receiver, common.FromHex(code))
