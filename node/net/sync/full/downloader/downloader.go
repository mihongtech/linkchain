...
func (d *Downloader) findAncestor(p *peerConnection, height uint64) (uint64, error) {
	...
	floor := int64(-1)
	if d.mode == FullSync {
		ceil = uint64(d.chain.GetBestBlock().GetHeight())
	}
	...
	for finished := false; !finished; {
		...
		case packet := <-d.blockCh:
			...
			for i := len(blocks) - 1; i >= 0; i-- {
				...
				if d.mode == FullSync && d.chain.HasBlock(*blocks[i].GetBlockID()) {
	...
	for start+1 < end {
		...
		for arrived := false; !arrived; {
			...
			case packer := <-d.blockCh:
				...
				if d.mode == FullSync && !d.chain.HasBlock(*blocks[0].GetBlockID()) {
					end = check
					break
				}
				start = check
...
func (d *Downloader) processBlocks(origin uint64, pivot uint64) error {
	...
	defer func() {
		if len(rollback) > 0 {
			...
			lastBlock := d.chain.GetBestBlock().GetHeight()

			// TODO: add rollback code
			// d.lightchain.Rollback(hashes)

			curBlock := d.chain.GetBestBlock().GetHeight()
...
func (d *Downloader) importBlockResults(results []*fetchResult) error {
	...
	for _, result := range results {
		...
		if !d.chain.HasBlock(*result.Block.GetPrevBlockID()) && !result.Block.GetPrevBlockID().IsEmpty() {
			continue
		}

		if d.chain.HasBlock(*result.Block.GetBlockID()) {
			continue
		}

		if err := d.chain.ProcessBlock(result.Block); err != nil {
