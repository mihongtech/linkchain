...
func TestSignWithPassphrase(t *testing.T) {
	...
	if _, unlocked := ks.unlocked[acc.Address.String()]; unlocked {
...
func TestWalletNotifications(t *testing.T) {
	...
	var (
		live       = make(map[string]accounts.Account)
	...
	for i := 0; i < 1024; i++ {
		if create := len(live) == 0 || rand.Int()%4 > 0; create {
			...
			live[account.Address.String()] = account
			...
			delete(live, account.Address.String())
...
func checkAccounts(t *testing.T, live map[string]accounts.Account, wallets []accounts.Wallet) {
	...
	for j, wallet := range wallets {
		...
		} else if !accs[0].IsEqual(&liveList[j]) {
...
func checkEvents(t *testing.T, want []walletEvent, have []walletEvent) {
	for _, wantEv := range want {
		...
		for ; len(have) > 0; nmatch++ {
			if have[0].Kind != wantEv.Kind || !have[0].a.IsEqual(&wantEv.a) {
				break
			}
			have = have[1:]
		}
...
func tmpKeyStore(t *testing.T, encrypted bool) (string, *KeyStore) {
	dstr, err := ioutil.TempDir("", "eth-keystore-test")
	if err != nil {
		t.Fatal(err)
	}
	if encrypted {
		return dstr, NewKeyStore(dstr, veryLightScryptN, veryLightScryptP)
	}
	return dstr, NewPlaintextKeyStore(dstr)
}
