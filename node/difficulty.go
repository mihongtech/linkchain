...
func HashToBig(hash *math.Hash) *big.Int {
...
func (bc *BlockChain) calcDifficulty(block *meta.Block) (uint32, error) {
	...
	} else if actualTimespan > config.MaxTimespan {
		adjustedTimespan = config.MaxTimespan
	}
	...
	if newTarget.Cmp(config.PowLimit) > 0 {
		newTarget.Set(newTarget)
	}

	powBits := BigToCompact(newTarget)
	if powBits < config.PowLimitBits {
		powBits = config.PowLimitBits
	}
