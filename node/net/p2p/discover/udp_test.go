...
import (
	...
	"github.com/mihongtech/linkchain/node/net/p2p/crypto"
...
func (test *udpTest) packetIn(wantError error, ptype byte, data packet) error {
	enc, _, err := encodePacket(test.remotekey, ptype, data.Serialize())
	if err != nil {
		return test.errorf("packet (%d) encode error: %v", ptype, err)
	}
	test.sent = append(test.sent, enc)
	if err = test.udp.handlePacket(test.remoteaddr, enc); err != wantError {
		return test.errorf("error mismatch: got %v, want %q, node %v", err, wantError, PubkeyID(&test.remotekey.PublicKey))
	}
	return nil
}
...
func TestUDP_findnode(t *testing.T) {
	...
	targetHash := math.HashH(testTarget[:])
