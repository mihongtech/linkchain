...
func TestCreate2Addreses(t *testing.T) {
	...
	for i, tt := range []testcase{
		...
		origin := meta.BytesToAccountID(common.FromHex(tt.origin))
		...
		*/
		expected := meta.BytesToAccountID(common.FromHex(tt.expected))
		if !bytes.Equal(expected.CloneBytes(), address.Bytes()) {
